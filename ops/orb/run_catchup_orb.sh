...
ORB_SESSION="${ORB_SESSION:-ieee_catchup_orb}"
...
set -euo pipefail
cd '$ORB_WORKDIR'
chmod +x ./ops/orb/orb_worker.sh
tmux kill-session -t '$ORB_SESSION' 2>/dev/null || true
tmux new-session -d -s '$ORB_SESSION' './ops/orb/orb_worker.sh ${FORWARDED_ARGS}'
echo 'Orb catch-up session started:' '$ORB_SESSION'
echo 'Log:' '$ORB_WORKDIR/catchup_orb.log'
"
